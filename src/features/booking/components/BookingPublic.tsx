...
function Shell({ step, children }: { step: number; children: React.ReactNode }) {
  ...
  return <div className="booking-v2-page"><section className="container booking-v2-shell"><header className="booking-v2-header"><div><span className="eyebrow">CASTRO’S · RESERVA</span><h1>Prepare a sua <em>reserva.</em></h1></div><p>O pedido só é enviado depois da revisão final.</p></header><ol className="booking-v2-steps" aria-label="Progresso da reserva">{labels.map((label, index) => <li key={label} className={index + 1 === step ? 'is-current' : index + 1 < step ? 'is-complete' : ''} aria-current={index + 1 === step ? 'step' : undefined}><span>{index + 1 < step ? <Check size={14} /> : String(index + 1).padStart(2, '0')}</span><strong>{label}</strong></li>)}</ol>{children}</section></div>
}
...
export function BookingDate() {
  ...
  const [draft, setDraft] = useState<BookingDraft>(() => {
    ...
    const fixedSchedule = target.type === 'COURSE_SESSION' && searchParams.get('fixed') === '1' && Boolean(date && duration)
    const next = { ...stored, date: stored.date ?? date, durationMinutes: stored.durationMinutes ?? duration, fixedSchedule: stored.fixedSchedule ?? fixedSchedule, participants: stored.participants ?? people, purpose: stored.purpose ?? (target.type === 'SPACE' ? purpose : undefined) }
  ...
  return <Shell step={1}><div className="booking-v2-grid"><main className="booking-v2-card"><div className="booking-v2-title"><CalendarDays size={21} /><div><span>01</span><h2>{draft.fixedSchedule ? 'Confirme a sessão selecionada.' : 'Quando pretende reservar?'}</h2></div></div><div className="booking-v2-fields"><TextField id="booking-date" label="Data" type="date" required disabled={draft.fixedSchedule} value={draft.date ?? ''} onChange={(event) => update({ date: event.target.value })} /><TextField id="booking-duration" label="Duração prevista (minutos)" type="number" min="1" required disabled={draft.fixedSchedule} value={draft.durationMinutes ?? ''} description={draft.fixedSchedule ? 'Data e duração correspondem à sessão publicada.' : 'Use a duração publicada quando existir ou ajuste apenas quando o serviço permitir.'} onChange={(event) => update({ durationMinutes: event.target.value ? Number(event.target.value) : undefined })} /></div></main><Summary target={target} draft={draft}><Button disabled={!canContinue} onClick={() => navigate(bookingRoute(target.type, target.id, 'time'))}>Ver horários <ArrowRight size={16} /></Button></Summary></div></Shell>
}
...
export function BookingConfirmation() { const api = useApi(); const { reference } = useParams(); const query = useQuery({ queryKey: ['booking', reference], enabled: Boolean(reference), queryFn: () => api.bookings.getByReference(reference!) }); return <div className="booking-v2-page"><section className="container booking-v2-confirmation"><span className="booking-v2-confirmation__icon"><Check size={26} /></span><span className="eyebrow">PEDIDO RECEBIDO</span><h1>Reserva registada.</h1>{query.isLoading && <LoadingState label="A carregar confirmação." />}{query.isError && <ErrorState title="A reserva foi enviada, mas não foi possível carregar os detalhes da confirmação." />}{query.data && <div className="booking-v2-confirmation__details"><div><small>Referência</small><strong>{query.data.reference}</strong></div><div><small>Estado</small><strong>{query.data.status}</strong></div><div><small>Horário</small><strong>{formatDateTime(query.data.startAt)} — {formatDateTime(query.data.endAt)}</strong></div></div>}<p>Guarde a referência para acompanhar este pedido.</p><Link className="ds-button ds-button--primary" to="/">Voltar ao início</Link></section></div> }

function Summary({ target, draft, children }: { target: { type: BookableType; id: string }; draft: BookingDraft; children: React.ReactNode }) { return <aside className="booking-v2-summary"><span className="eyebrow">RESUMO</span><h2>Pedido de reserva</h2><dl><div><dt>Tipo</dt><dd>{targetLabel(target.type)}</dd></div>{draft.purpose && <div><dt>Finalidade</dt><dd>{purposeLabel(draft.purpose)}</dd></div>}{draft.participants && <div><dt>Participantes</dt><dd>{draft.participants}</dd></div>}{draft.date && <div><dt>Data</dt><dd>{draft.date}</dd></div>}{draft.durationMinutes && <div><dt>Duração</dt><dd>{draft.durationMinutes} min</dd></div>}{draft.startTime && <div><dt>Horário</dt><dd>{formatTime(draft.startTime)}{draft.endTime ? `–${formatTime(draft.endTime)}` : ''}</dd></div>}</dl><div className="booking-v2-summary__action">{children}</div><small>A disponibilidade e o estado final são determinados pelo sistema de reservas.</small></aside> }
