...
export function BookingDate() {
  ...
  const update = (patch: Partial<BookingDraft>) => { const next = { ...draft, ...patch, startTime: undefined, endTime: undefined }; setDraft(next); writeDraft(target.type, target.id, next) }
...
export function BookingTime() {
  ...
  const select = (start: string, end: string) => { const next = { ...draft, startTime: start, endTime: end }; setDraft(next); writeDraft(target.type, target.id, next) }
...
export function BookingCustomer() {
  ...
  const update = (patch: Partial<BookingDraft>) => { const next = { ...draft, ...patch }; setDraft(next); writeDraft(target.type, target.id, next) }
...
export function BookingReview() {
  ...
  const mutation = useMutation({ mutationFn: async () => {
    ...
    const request: BookingRequestDto = { bookableType: target.type, bookableId: target.id, date: draft.date, startTime: draft.startTime, endTime: draft.endTime, participants: draft.participants, customer: { firstName: draft.firstName, lastName: draft.lastName, email: draft.email, phone: draft.phone }, notes: draft.notes }
    return api.bookings.create(request)
...
function Summary({ target, draft, children }: { target: { type: BookableType; id: string }; draft: BookingDraft; children: React.ReactNode }) {
  return <aside className="booking-v2-summary"><span className="eyebrow">RESUMO</span><h2>Pedido de reserva</h2><dl><div><dt>Tipo</dt><dd>{targetLabel(target.type)}</dd></div>{draft.date && <div><dt>Data</dt><dd>{draft.date}</dd></div>}{draft.durationMinutes && <div><dt>Duração</dt><dd>{draft.durationMinutes} min</dd></div>}{draft.startTime && <div><dt>Horário</dt><dd>{formatTime(draft.startTime)}{draft.endTime ? `–${formatTime(draft.endTime)}` : ''}</dd></div>}</dl><div className="booking-v2-summary__action">{children}</div><small>A disponibilidade e o estado final são determinados pelo sistema de reservas.</small></aside>
}
