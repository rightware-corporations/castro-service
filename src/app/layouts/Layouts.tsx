...
export function PublicLayout() {
  ...
  return <div className="public-layout public-v2-shell"><header className="public-header"><div className="container public-header__inner"><Brand /><nav className={`public-nav ${menuOpen ? 'public-nav--open' : ''}`} aria-label="Navegação principal">{publicLinks.map((link) => <NavLink key={link.to} to={link.to} onClick={() => setMenuOpen(false)}>{link.label}</NavLink>)}<Link className="button button--nav public-header__cta" to="/contacto" onClick={() => setMenuOpen(false)}>Falar connosco <ChevronRight size={16} /></Link></nav><button className="icon-button public-menu-toggle" type="button" onClick={() => setMenuOpen((open) => !open)} aria-label={menuOpen ? 'Fechar menu' : 'Abrir menu'} aria-expanded={menuOpen}>{menuOpen ? <X size={22} /> : <Menu size={22} />}</button></div></header><main><Outlet /></main><footer className="public-footer public-footer--v2"><div className="container public-footer__v2-grid"><div className="public-footer__brand-block"><Brand inverse /><p>Consultoria, formação e espaços reunidos numa experiência pensada para pessoas e organizações.</p></div><nav className="public-footer__nav" aria-label="Navegação do rodapé"><span className="public-footer__label">Explorar</span>{publicLinks.map((link) => <Link key={link.to} to={link.to}>{link.label}<ArrowUpRight size={14} aria-hidden="true" /></Link>)}</nav><div className="public-footer__closing"><span className="public-footer__label">Castro’s Services</span><p>Atendimento ao Cliente · Ética e Liderança Organizacional · Palestras, Workshops e Formação · Treinamento Corporativo Personalizado.</p></div></div><div className="container public-footer__baseline"><span>Castro’s Services</span><span>Experiência digital em desenvolvimento.</span></div></footer></div>
}

export function AuthLayout() { return <div className="auth-layout"><header className="auth-layout__header"><Brand inverse /></header><main><Outlet /></main></div> }
