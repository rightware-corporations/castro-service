  ...
  return (
    <section className="ops-v2">
      <header className="ops-v2__hero">
        ...
        <div className="ops-v2__environment" aria-label="Estado do ambiente"><span className={api.kind === 'mock' ? 'is-development' : 'is-connected'} /><div><small>Dados</small><strong>{api.kind === 'mock' ? 'Modo de desenvolvimento' : 'Backend ligado'}</strong></div></div>
      </header>
...
function Dashboard({ visibleModules }: { visibleModules: typeof modules }) {
  return <>
    ...
    <section className="ops-v2__modules"><div className="ops-v2__section-heading"><div><span className="eyebrow">WORKSPACE</span><h2>Áreas de trabalho</h2></div><p>Entradas principais da operação, sem dados fictícios.</p></div><div className="ops-v2__module-grid">{visibleModules.map(({ to, label, description, icon: Icon }) => <Link key={to} to={to}><span className="ops-v2__module-icon"><Icon size={20} /></span><div><strong>{label}</strong><p>{description}</p></div><ArrowRight size={18} /></Link>)}</div></section>
  </>
}

function RequestsWorkspace() {
  const can = useCan()
  return <section className="ops-workspace"><WorkspaceToolbar searchPlaceholder="Pesquisar por nome, email ou referência" action={can('request.create') ? <button type="button" className="ds-button ds-button--primary"><Plus size={16} /> Novo pedido</button> : undefined} /><div className="ops-workspace__tabs" role="tablist" aria-label="Estado dos pedidos"><button type="button" role="tab" aria-selected="true">Todos</button><button type="button" role="tab">Novos</button><button type="button" role="tab">Em acompanhamento</button><button type="button" role="tab">Fechados</button></div><div className="ops-workspace__table-shell"><div className="ops-workspace__table-head" aria-hidden="true"><span>Cliente</span><span>Tipo</span><span>Responsável</span><span>Estado</span><span>Recebido</span></div><OperationalCollectionEmpty icon={Inbox} title="Ainda não existem pedidos para mostrar." description="Quando o backend operacional expuser pedidos autenticados, esta vista receberá pesquisa, filtros, estado e atribuição sem mudar a composição." /></div></section>
}

function BookingsWorkspace() {
  const can = useCan()
  return <section className="ops-workspace"><WorkspaceToolbar searchPlaceholder="Pesquisar por referência ou cliente" action={can('booking.create') ? <button type="button" className="ds-button ds-button--primary"><Plus size={16} /> Nova reserva</button> : undefined} /><div className="ops-workspace__summary-strip"><span><small>Hoje</small><strong>—</strong></span><span><small>Pendentes</small><strong>—</strong></span><span><small>Confirmadas</small><strong>—</strong></span><span><small>Canceladas</small><strong>—</strong></span></div><div className="ops-workspace__table-shell"><div className="ops-workspace__table-head ops-workspace__table-head--bookings" aria-hidden="true"><span>Referência</span><span>Cliente</span><span>Reserva</span><span>Data</span><span>Estado</span></div><OperationalCollectionEmpty icon={ClipboardList} title="Sem reservas operacionais ligadas." description="O fluxo público já cria reservas reais; falta apenas o contrato autenticado para listar e gerir essas reservas nesta área interna." /></div></section>
}

function CustomersWorkspace() {
  return <section className="ops-workspace"><WorkspaceToolbar searchPlaceholder="Pesquisar clientes por nome, email ou telefone" /><div className="ops-workspace__summary-strip"><span><small>Clientes</small><strong>—</strong></span><span><small>Com pedidos</small><strong>—</strong></span><span><small>Com reservas</small><strong>—</strong></span><span><small>Atualizados</small><strong>—</strong></span></div><div className="ops-workspace__table-shell"><div className="ops-workspace__table-head" aria-hidden="true"><span>Cliente</span><span>Contacto</span><span>Última interação</span><span>Origem</span><span>Estado</span></div><OperationalCollectionEmpty icon={UsersRound} title="A base de clientes ainda não está ligada." description="O modelo visual está pronto para consolidar pedidos e reservas por cliente quando o endpoint autenticado estiver disponível." /></div></section>
}

function TasksWorkspace() {
  return <section className="ops-workspace"><WorkspaceToolbar searchPlaceholder="Pesquisar tarefas e seguimentos" action={<button type="button" className="ds-button ds-button--primary"><Plus size={16} /> Nova tarefa</button>} /><div className="ops-workspace__tabs" role="tablist" aria-label="Estado das tarefas"><button type="button" role="tab" aria-selected="true">Todas</button><button type="button" role="tab">Hoje</button><button type="button" role="tab">Próximas</button><button type="button" role="tab">Concluídas</button></div><div className="ops-workspace__table-shell"><div className="ops-workspace__table-head" aria-hidden="true"><span>Tarefa</span><span>Relacionada com</span><span>Responsável</span><span>Prazo</span><span>Estado</span></div><OperationalCollectionEmpty icon={CheckSquare2} title="Sem tarefas operacionais publicadas." description="A área de follow-up está pronta para receber tarefas reais ligadas a pedidos, reservas e clientes." /></div></section>
}

function CalendarWorkspace() {
  return <section className="ops-workspace"><div className="ops-calendar-toolbar"><div className="ops-calendar-toolbar__nav"><button type="button" aria-label="Período anterior">‹</button><button type="button">Hoje</button><button type="button" aria-label="Período seguinte">›</button></div><strong>Agenda operacional</strong><div className="ops-calendar-toolbar__views"><button type="button">Dia</button><button type="button">Semana</button><button type="button" className="is-active">Mês</button></div></div><div className="ops-calendar-grid" aria-label="Calendário mensal sem dados operacionais">{['SEG', 'TER', 'QUA', 'QUI', 'SEX', 'SÁB', 'DOM'].map((day) => <div className="ops-calendar-grid__weekday" key={day}>{day}</div>)}{Array.from({ length: 35 }, (_, index) => <div className="ops-calendar-grid__day" key={index}><span>{index + 1 <= 31 ? index + 1 : ''}</span></div>)}</div><p className="ops-calendar-note"><ShieldCheck size={16} /> Nenhum evento é simulado. Reservas e bloqueios aparecerão quando o endpoint interno autenticado estiver disponível.</p></section>
}
