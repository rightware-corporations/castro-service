...
public class SpaceAdminController {
    ...
    public SpaceAdminController(SpaceRepository spaces) {
        this.spaces = spaces;
    }
    ...
    public List<SpaceResponse> list(Authentication authentication) {
        return spaces.findAllByOrganizationIdOrderByNameAsc(organizationId(authentication)).stream().map(this::toResponse).toList();
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    @PreAuthorize("hasAuthority('space.manage')")
    public SpaceResponse create(@Valid @RequestBody SpaceInput input, Authentication authentication) {
        ...
        apply(space, input, organizationId, slug);
        return toResponse(spaces.save(space));
    }

    @PutMapping("/{id}")
    @PreAuthorize("hasAuthority('space.manage')")
    public SpaceResponse update(@PathVariable UUID id, @Valid @RequestBody SpaceInput input, Authentication authentication) {
        UUID organizationId = organizationId(authentication);
        validateCapacity(input.capacityMin(), input.capacityMax());
    ...
    public void deactivate(@PathVariable UUID id, Authentication authentication) {
        Space space = spaces.findByOrganizationIdAndId(organizationId(authentication), id)
            .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Space not found"));
        space.active = false;
        spaces.save(space);
    }
    ...
    private void ensureSlugAvailable(UUID organizationId, String slug, UUID currentId) {
        spaces.findByOrganizationIdAndSlug(organizationId, slug).ifPresent(existing -> {
            if (currentId == null || !existing.id.equals(currentId)) throw new ResponseStatusException(HttpStatus.CONFLICT, "Space slug already exists");
        });
    }

    private void apply(Space space, SpaceInput input, UUID organizationId, String slug) {
        space.organizationId = organizationId;
        space.name = input.name().trim();
        space.slug = slug;
        space.description = clean(input.description());
        space.location = clean(input.location());
        space.capacityMin = input.capacityMin();
        space.capacityMax = input.capacityMax();
        space.sizeSquareMeters = input.sizeSquareMeters();
        space.active = input.active();
    }

    private SpaceResponse toResponse(Space space) {
        return new SpaceResponse(space.id, space.name, space.slug, space.description, space.location,
            space.capacityMin, space.capacityMax, space.sizeSquareMeters, space.active);
    }

    private static void validateCapacity(Integer min, Integer max) {
        if (min != null && max != null && min > max) throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Minimum capacity cannot exceed maximum capacity");
    }
    ...
    public record SpaceInput(
        ...
        String description,
        String location,
        @Min(0) Integer capacityMin,
        @Min(0) Integer capacityMax,
        @Positive BigDecimal sizeSquareMeters,
        boolean active
    ) {}
