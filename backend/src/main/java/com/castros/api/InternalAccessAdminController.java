...
public class InternalAccessAdminController {
    ...
    private UserItem userItem(UUID org,UUID id){ return jdbc.query("""select u.id,u.email,u.first_name,u.last_name,u.active,u.created_at,r.id role_id,r.name role_name from users u left join organization_members om on om.user_id=u.id and om.organization_id=u.organization_id left join roles r on r.id=om.role_id where u.organization_id=? and u.id=?""",rs->{if(!rs.next())throw new ResponseStatusException(HttpStatus.NOT_FOUND,"User not found"); return new UserItem(rs.getObject("id",UUID.class),rs.getString("email"),rs.getString("first_name"),rs.getString("last_name"),rs.getBoolean("active"),rs.getObject("created_at",OffsetDateTime.class),rs.getObject("role_id",UUID.class),rs.getString("role_name"));},org,id); }
