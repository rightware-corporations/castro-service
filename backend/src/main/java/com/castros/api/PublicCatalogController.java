...
public class PublicCatalogController {
    ...
    @GetMapping("/services") @Operation(summary="List active services")
    public List<CatalogItem> services() { return services.findByActiveTrueOrderBySortOrderAsc().stream().map(s -> new CatalogItem(s.id,s.name,s.slug,s.description,s.durationMinutes,s.bookingEnabled,s.confirmationMode)).toList(); }
    @GetMapping("/services/{slug}")
    public CatalogItem service(@PathVariable String slug) { var s=services.findBySlugAndActiveTrue(slug).orElseThrow(); return new CatalogItem(s.id,s.name,s.slug,s.description,s.durationMinutes,s.bookingEnabled,s.confirmationMode); }
    ...
    @GetMapping("/spaces") public List<SpaceItem> spaces() { return spaces.findByActiveTrueOrderByNameAsc().stream().map(s -> new SpaceItem(s.id,s.name,s.slug,s.description,s.location,s.capacityMin,s.capacityMax,s.bookingEnabled,s.confirmationMode)).toList(); }
    @GetMapping("/spaces/{slug}") public SpaceItem space(@PathVariable String slug) { var s=spaces.findBySlugAndActiveTrue(slug).orElseThrow(); return new SpaceItem(s.id,s.name,s.slug,s.description,s.location,s.capacityMin,s.capacityMax,s.bookingEnabled,s.confirmationMode); }
}
