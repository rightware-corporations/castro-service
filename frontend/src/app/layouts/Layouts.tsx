...
function PublicBrand() {
  return <Link className="brand-mark" to="/" aria-label="Castro’s Services — início"><img className="public-brand-image" src="/castros-logo-original.jpg" alt="" width="192" height="48" /></Link>
}
...
export function PublicLayout() {
  ...
  return <div className="public-layout public-v2-shell" data-shell={shell.id} data-family={shell.family}><SkipLink /><header className={`public-header ${scrolled ? 'public-header--scrolled' : ''}`} data-scroll-state={scrolled ? 'scrolled' : 'top'}><div className="container public-header__inner"><PublicBrand /><motion.nav id="public-primary-navigation" className={`public-nav ${menuOpen ? 'public-nav--open' : ''}`} aria-label="Navegação principal" aria-hidden={isMobile && !menuOpen ? true : undefined} inert={isMobile && !menuOpen} initial={false} animate={isMobile ? (menuOpen ? 'open' : 'closed') : 'desktop'} variants={motionPresets.mobileMenu}>{publicLinks.map((link) => <NavLink key={link.to} to={link.to} onClick={() => setMenuOpen(false)}>{link.label}</NavLink>)}<Link className="button button--nav public-header__cta" to="/contacto" onClick={() => setMenuOpen(false)}>Falar connosco <ChevronRight size={16} /></Link></motion.nav><button ref={menuTriggerRef} className="icon-button public-menu-toggle" type="button" onClick={() => setMenuOpen((open) => !open)} aria-label={menuOpen ? 'Fechar menu' : 'Abrir menu'} aria-expanded={menuOpen} aria-controls="public-primary-navigation">{menuOpen ? <X size={22} /> : <Menu size={22} />}</button></div></header><AnimatePresence initial={false}>{menuOpen && isMobile ? <motion.button className="public-nav-scrim" type="button" onClick={closeMenu} aria-label="Fechar menu ao tocar fora da navegação" initial={motionPresets.scrim.initial} animate={motionPresets.scrim.animate} exit={motionPresets.scrim.exit} transition={publicMotion.crossfade.transition} /> : null}</AnimatePresence><main ref={mainRef} id="main-content" tabIndex={-1}><Outlet /></main><footer className="public-footer public-footer--v2"><div className="container public-footer__v2-grid"><div className="public-footer__brand-block"><PublicBrand /><p>Consultoria, formação e espaços reunidos numa experiência pensada para pessoas e organizações.</p></div><nav className="public-footer__nav" aria-label="Navegação do rodapé"><span className="public-footer__label">Explorar</span>{publicLinks.map((link) => <Link key={link.to} to={link.to}>{link.label}<ArrowUpRight size={14} aria-hidden="true" /></Link>)}</nav><div className="public-footer__closing"><span className="public-footer__label">Castro’s Services</span><p>Atendimento ao Cliente · Ética e Liderança Organizacional · Palestras, Workshops e Formação · Treinamento Corporativo Personalizado.</p></div></div><div className="container public-footer__baseline"><span>Castro’s Services</span><span>Consultoria · Formação · Espaços</span></div></footer></div>
}
...
export function OperationsLayout() {
  ...
  const closeSidebar = () => { setSidebarOpen(false); menuTriggerRef.current?.focus() }
  useEscapeToClose(sidebarOpen, isMobile, closeSidebar)
  ...
  return <div className="operations-layout"><SkipLink /><motion.aside id="operations-navigation-panel" className={`operations-sidebar ${sidebarOpen ? 'operations-sidebar--open' : ''}`} aria-hidden={isMobile && !sidebarOpen ? true : undefined} inert={isMobile && !sidebarOpen} initial={false} animate={isMobile ? (sidebarOpen ? 'open' : 'closed') : 'desktop'} variants={motionPresets.mobileSidebar}><div className="operations-sidebar__top"><Brand inverse /><button className="icon-button operations-close" type="button" onClick={closeSidebar} aria-label="Fechar navegação"><X size={20} /></button></div><p className="operations-sidebar__label">CASTRO’S OPERATIONS</p><nav className="operations-nav" aria-label="Navegação de operações">{visibleLinks.length ? visibleLinks.map(({ to, label, icon: Icon }) => <NavLink key={to} to={to} onClick={() => setSidebarOpen(false)}><Icon size={18} />{label}</NavLink>) : <span className="operations-nav__empty">Sem permissões atribuídas.</span>}</nav><div className="operations-sidebar__bottom"><Link to="/"><ChevronRight size={15} />Voltar ao site</Link><button type="button" onClick={logout} disabled={loggingOut}><LogOut size={15} />{loggingOut ? 'A sair…' : 'Terminar sessão'}</button></div></motion.aside><AnimatePresence initial={false}>{sidebarOpen && isMobile ? <motion.button className="operations-scrim" type="button" onClick={closeSidebar} aria-label="Fechar navegação" initial={motionPresets.scrim.initial} animate={motionPresets.scrim.animate} exit={motionPresets.scrim.exit} transition={motionPresets.scrim.transition} /> : null}</AnimatePresence><div className="operations-main"><header className="operations-topbar"><button ref={menuTriggerRef} className="icon-button operations-menu" type="button" onClick={() => setSidebarOpen(true)} aria-label="Abrir navegação" aria-expanded={sidebarOpen} aria-controls="operations-navigation-panel"><Menu size={21} /></button><div><span>Área interna</span><strong>{session?.displayName || 'Secretária · Castro’s Services'}</strong></div><span className="topbar-account">Operações</span></header><main id="main-content" tabIndex={-1} className="operations-content"><Outlet /></main><nav className="mobile-bottom-nav" aria-label="Navegação rápida">{visibleLinks.slice(0, 4).map(({ to, label, icon: Icon }) => <NavLink key={to} to={to}><Icon size={18} /><span>{label}</span></NavLink>)}</nav></div></div>
}
