...
describe('public layout navigation', () => {
  ...
  it('keeps the /app mobile sidebar keyboard-operable without changing route permissions', async () => {
    ...
    const operationsNav = await screen.findByRole('navigation', { name: 'Navegação de operações' })
    expect(within(operationsNav).getByRole('link', { name: /Dashboard/i })).toHaveAttribute('href', '/app/dashboard')
    await user.keyboard('{Escape}')
    expect(trigger).toHaveAttribute('aria-expanded', 'false')
